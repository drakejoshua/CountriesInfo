...
// add an event listener to the search-form in order to do the 
// in-page fetch from the API without reloading
document.forms[0].addEventListener( 'submit', searchCountriesData );

// the searchCountriesData() 
// this function acts as an event handler used to async sanitize the search-form data, 
// do the searching from the API and display the results on the page
function searchCountriesData( event ) {

    // prevent the reloading of the page( the form default submit behaviour )
    event.preventDefault();

    // validate search-input.. if valid, search for input value, if not, alert user that input is invalid
    if ( searchInput.value == '' ) {

        alert('Enter a search value to get results');

    } else {

        // change the body UI state to the loading state
        bodyStatefulRepresentation.internalState = { type: 'loading' };

        // perform the search from the API, paginate the returned data and display the data on the page
        fetchCountriesData( filterSelect.value, searchInput.value )
        .then(
            function( data ) {
                if ( data != 'error' ) {
                    // change the body UI state to the data-loaded state
                    bodyStatefulRepresentation.internalState = { type: 'data' };

                    // paginate fetch data
                    paginatedAppData = paginateData( 12, data );
                    pageNumber = 1;
        
                    // display paginated data
                    displayData();
                } else {
                    // change the body UI state to the error state due to error when loading
...
function displayData() {
    ...
    if ( paginatedAppData.length != 0 ) {
        ...
        for ( var displayData of paginatedAppData[ ( pageNumber - 1 ) ] ) {
            ...
            // check if the current country-info name is in the imported list or not
            // in order to determine whether the inserted template should be starred or not
            if ( starredCountriesNames.includes( displayData.name.common ) ) {
                countryInfoTemplate.insert( displayData, 'starred' );
            } else {
                countryInfoTemplate.insert( displayData, 'unstarred' );
            }
        }
